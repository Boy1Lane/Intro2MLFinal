...
Sơ đồ cấu trúc chi tiết của mô hình và classification head được thể hiện qua Hình~\ref{fig:phobert_arch}.
...
\begin{tikzpicture}[
    node distance=1.0cm,
    block/.style={draw, rectangle, rounded corners, align=center, fill=blue!5, font=\small, minimum width=6cm, minimum height=0.7cm},
    head_block/.style={draw, rectangle, rounded corners, align=center, fill=orange!10, font=\small, minimum width=6cm, minimum height=0.7cm},
    arrow/.style={thick, ->, >=stealth}
]
    % Nodes
    \node (input) [block, fill=gray!10] {Input Tokens \\ (max\_length = 256)};
    \node (emb) [block, below=of input] {Embedding Layer \\ (Word + Position + Segment)};
    \node (enc) [block, below=of emb, fill=blue!10] {12x Transformer Encoder Layers \\ (12 attention heads, $d_{model} = 768$)};
    \node (cls) [block, below=of enc] {[CLS] Token Representation \\ (Vector size: 768)};
    \node (dense) [head_block, below=of cls] {Dense Layer (768 $\rightarrow$ 768) \\ + Tanh Activation};
    \node (linear) [head_block, below=of dense] {Linear Classification Head \\ (768 $\rightarrow$ 3 classes)};
    \node (softmax) [block, below=of linear, fill=green!5] {Softmax Function \\ (CLEAN, OFFENSIVE, HATE)};
    ...
    \draw [arrow] (dense) -- (linear);
    \draw [arrow] (linear) -- (softmax);
\end{tikzpicture}
\caption{Sơ đồ kiến trúc chi tiết mô hình PhoBERT-base-v2 và classification head}
\label{fig:phobert_arch}
\end{figure}
...
Dữ liệu mất cân bằng nghiêm trọng (CLEAN: 82\%, OFFENSIVE: 7\%, HATE: 11\%), nhóm áp dụng ba chiến lược:
\begin{itemize}
