...
def load_datasets(data_dir="."):
    ...
    for name in ["train", "dev", "test"]:
        ...
        print(
            f"   Label distribution: {df['label_id'].value_counts().sort_index().to_dict()}"
        )
        print()
    return datasets
...
def process_dataset(df, name="dataset"):
    ...
    print(
        f"   Độ dài text trung bình: {df['free_text_clean'].str.len().mean():.1f} ký tự"
    )
    print(
        f"   Số từ trung bình: {df['free_text_clean'].str.split().str.len().mean():.1f} từ"
    )
...
if __name__ == "__main__":
    ...
    for name in ["train", "dev", "test"]:
        ...
        removed = orig - clean
        print(
            f"   {name}: {orig} → {clean} (xóa {removed} dòng, -{removed/orig*100:.1f}%)"
        )
